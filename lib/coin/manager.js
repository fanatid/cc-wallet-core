 ...
 * @param {Wallet} wallet
 */
function CoinManager (storage, wallet) {
  var self = this
 ...
 * @return {Promise}
 */
CoinManager.prototype._addTx = function (info, emit) {
  var self = this
    ...
    return self._wallet.getAllAddresses()
      .then(function (walletAddresses) {
        ...
        return Promise.map(tx.outputs, function (output, oidx) {
          ...
          return self._storage.get(info.txid, oidx)
        ...
        .then(function (touched) {
          _.uniq(_.flatten(touched)).forEach(function (address) {
            emit('touchAddress', address, info.txid)
          })
        })
      })
      .then(function () {
        // @todo calculate only for own inputs and outputs!
        // calculate color values and generate touchAsset
        return self._getAssets(tx)
      })
      .then(function (adefs) {
        adefs.forEach(function (adef) {
          emit('touchAsset', adef, info.txid)
        })
      })
  })
}
 ...
 * @return {Promise}
 */
CoinManager.prototype.loadTx = function (info) {
  return this._addTx(info, _.noop)
}
 ...
 * @return {Promise}
 */
CoinManager.prototype.addTx = function (info) {
  return this._addTx(info, this.emit.bind(this))
}
 ...
 * @return {Promise}
 */
CoinManager.prototype.removeTx = function (info) {
  var self = this
  return self._withLock(function () {
    var tx = bitcore.Transaction(info.rawtx)
    ...
    return self._wallet.getAllAddresses()
      .then(function (walletAddresses) {
        var removedCoins = _.filter(self._coins, {txid: info.txid})
        return Promise.map(removedCoins, function (coin) {
          return self._storage.remove(coin.txid, coin.oidx)
        })
        .then(function () {
          self._coins = _.reject(self._coins, {txid: info.txid})
          ...
          _.chain(removedCoins)
            ...
            .forEach(function (address) {
              self.emit('touchAddress', address, info.txid)
            })
      ...
      .then(function (adefs) {
        adefs.forEach(function (adef) {
          self.emit('touchAsset', adef, info.txid)
        })
      })
  })
}
