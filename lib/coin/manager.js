 ...
 * @param {number} [opts.timestamp] Freeze until timestamp is not be reached (in seconds)
 * @param {number} [opts.fromNow] As a timestamp that equal (Math.round(Date.now() / 1000) + fromNow)
 * @throws {CoinNotFoundError} If coin for given txId:outIndex not found
 */
CoinManager.prototype.freezeCoins = function (coins, opts) {
  var lockTime = opts.height || opts.timestamp || opts.fromNow + Math.round(Date.now() / 1000)
  if (lockTime < 0) {
