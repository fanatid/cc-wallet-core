...
function TxManager (storage, wallet) {
  ...
  var notifyNeedSync = _.debounce(function () {
    ...
    promise = promise
      .then(function () {
        return self._wallet.isInitialized()
      })
...
TxManager.prototype._syncUnknowTxs = function () {
  ...
  return self._wallet.getAllAddresses()
    .then(function (addresses) {
      ...
      return self._wallet.blockchain.addressesQuery(addresses, opts)
    })
    .then(function (data) {
      ...
      return Promise.map(txids, function (txid) {
        ...
        return Promise.all([
          ...
          self._wallet.blockchain.getTxBlockHash(txid)
        ])
...
TxManager.prototype.getTx = function (txid) {
  // without _withLock because getTxFn depends from this and uses in CoinManager._addTx
  var record = this._indexedTxRecords[txid]
  return Promise.resolve(record === undefined ? null : record.rawtx)
}
