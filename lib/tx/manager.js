    ...
    return self._storage.get()
      .then(function (records) {
        ...
        return Promise.map(self._txRecords, function (record) {
          var tx = bitcore.Transaction(record.rawtx)
          return self._wallet.coinManager.loadTx(tx)
        })
      })
      .finally(function () {
        self._syncExit()
...
TxManager.prototype._attemptSendTx = function (txid, attempt) {
  ...
  return self.ready
    .then(function () {
      ...
      function updateTxStatus (status) {
        ...
        return self._storage.update(txid, {status: status})
          .then(function () {
            ...
            return self._wallet.coinManager.removeTx(tx)
          })
          .then(function () {
      ...
      self._syncEnter()
      return self._wallet.withStateLock(function () {
  ...
  return self._wallet.getAllAddresses()
    ...
    .then(function (data) {
      ...
      return Promise.map(txids, function (txid) {
        ...
        .then(function () {
          var tx = bitcore.Transaction(record.rawtx)
          return self._wallet.coinManager.addTx(tx)
        })
        .then(function () {
          record.txid = txid
    ...
    .then(function () {
      ...
      return self._wallet.withStateLock(function () {
        return self._storage.add(txid, data)
          .then(function () {
            self._wallet.coinManager.addTx(tx)
          })
          .then(function () {
            data.txid = txid
