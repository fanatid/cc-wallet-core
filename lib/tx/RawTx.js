...
RawTx.prototype.satisfiesDeltas = function (wallet, deltas, maxfee, allowExtra, cb){
  if (_.isFunction(allowExtra) && _.isUndefined(cb)) {
    cb = allowExtra
    allowExtra = undefined
  }
  ...
  this.getDeltaColorValues(wallet, function(error, txDeltas){
    ...
    // check values
    var error = null
    _.keys(deltasMap).foreach(function(colorId){
      if(error){ return }
      var uncolored = colorId == 0
      var value = deltasMap[colorId].getValue()
      var txValue = txDeltasMap[colorId].getValue()
      if(uncolored && ((value < txValue) || (value > (txValue + maxfee)))){
