...
var SQL = {
  ...
  update: {
    value: 'UPDATE ccwallet_config SET value = $1 WHERE key = $2'
  },
  select: {
...
_.extend(AbstractSQLConfigStorage, IConfigStorage)
  ...
  return self._storage.withLock(function () {
    return self._storage.executeSQL(SQL.select.value, [key])
      .then(function (rows) {
        ...
        if (rows.length === 0) {
          return self._storage.executeSQL(SQL.insert.value, [key, value])
        }

        return self._storage.executeSQL(SQL.update.value, [value, key])
      })
  })
  .then(_.noop)
}
