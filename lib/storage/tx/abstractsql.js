 ...
 */
AbstractSQLTxStorage.prototype._getRow = function (txid) {
  return this._storage.executeSQL(SQL.select.record, [txid])
    .then(function (rows) {
      if (rows.length === 0) {
        return null
      }
      ...
      var data = rows[0]
      data.txid = txid
      data.isBlockTimestamp = !!data.isBlockTimestamp
      return data
    })
}
  ...
  return self._storage.withLock(function () {
    return self._getRow(txid)
      .then(function (record) {
        ...
        data = _.defaults(_.clone(data), record)
        var args = [
          data.rawtx,
          data.status,
          data.blockHeight || null,
