...
var SQL = {
  create: {
    assets: 'CREATE TABLE IF NOT EXISTS ccwallet_definitions ( ' +
            '  pk INTEGER PRIMARY KEY AUTOINCREMENT, ' +
            '  id TEXT NOT NULL UNIQUE, ' +
            '  unit TEXT NOT NULL)',
    monikers: 'CREATE TABLE IF NOT EXISTS ccwallet_definitions_monikers ( ' +
              '  moniker TEXT PRIMARY KEY, ' +
              '  asset_id INTEGER NOT NULL, ' +
              '  FOREIGN KEY (asset_id) REFERENCES ccwallet_definitions(pk))',
    cdescs: 'CREATE TABLE IF NOT EXISTS ccwallet_definitions_cdescs ( ' +
              '  cdesc TEXT PRIMARY KEY, ' +
              '  asset_id INTEGER NOT NULL, ' +
              '  FOREIGN KEY (asset_id) REFERENCES ccwallet_definitions(pk))'
  },
  insert: {
    asset: 'INSERT INTO ccwallet_definitions ' +
           '  (id, unit) VALUES ($1, $2)',
    moniker: 'INSERT INTO ccwallet_definitions_monikers ' +
             '  (moniker, asset_id) VALUES ($1, $2)',
    cdesc: 'INSERT INTO ccwallet_definitions_cdescs ' +
           '  (cdesc, asset_id) VALUES ($1, $2)'
  },
  select: {
    assetPK: 'SELECT pk FROM ccwallet_definitions WHERE id = $1',
    all: 'SELECT * FROM ccwallet_definitions ' +
         '  JOIN ccwallet_definitions_monikers ' +
         '    ON ccwallet_definitions.pk = ccwallet_definitions_monikers.asset_id' +
         '  JOIN ccwallet_definitions_cdescs ' +
         '    ON ccwallet_definitions.pk = ccwallet_definitions_cdescs.asset_id ',
    byId: 'SELECT * FROM ccwallet_definitions ' +
          '  JOIN ccwallet_definitions_monikers ' +
          '    ON ccwallet_definitions.pk = ccwallet_definitions_monikers.asset_id ' +
          '  JOIN ccwallet_definitions_cdescs ' +
          '    ON ccwallet_definitions.pk = ccwallet_definitions_cdescs.asset_id' +
          '  WHERE ccwallet_definitions.id = $1',
    byMoniker: 'SELECT * FROM ccwallet_definitions ' +
               '  JOIN ccwallet_definitions_monikers ' +
               '    ON ccwallet_definitions.pk = ccwallet_definitions_monikers.asset_id ' +
               '  JOIN ccwallet_definitions_cdescs ' +
               '    ON ccwallet_definitions.pk = ccwallet_definitions_cdescs.asset_id ' +
               '  WHERE ccwallet_definitions.pk = (SELECT asset_id FROM ccwallet_definitions_monikers WHERE moniker = $1)',
    byCdesc: 'SELECT * FROM ccwallet_definitions ' +
             '  JOIN ccwallet_definitions_monikers ' +
             '    ON ccwallet_definitions.pk = ccwallet_definitions_monikers.asset_id ' +
             '  JOIN ccwallet_definitions_cdescs ' +
             '    ON ccwallet_definitions.pk = ccwallet_definitions_cdescs.asset_id ' +
             '  WHERE ccwallet_definitions.pk = (SELECT asset_id FROM ccwallet_definitions_cdescs WHERE cdesc = $1)'
  },
  delete: {
    all: {
      assets: 'DELETE FROM ccwallet_definitions',
      monikers: 'DELETE FROM ccwallet_definitions_monikers',
      cdescs: 'DELETE FROM ccwallet_definitions_cdescs'
    }
  }
}
