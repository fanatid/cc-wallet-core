...
function WalletEventNotifier (wallet) {
  ...
  this.setMaxListeners(0) // unlimited, as we dynamically add them via once
...
WalletEventNotifier.prototype._addEventListeners = function () {
  ...
  setInterval(function () {
    if (!self._connected && self._canSync()) {
      notifyNeedsSync()
      var bc = self._wallet.getBlockchain()
      if (bc._syncLatest) bc._syncLatest()
    }
  }, 5000)
