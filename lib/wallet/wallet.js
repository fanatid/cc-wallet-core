...
function Wallet (opts) {
  ...
  if (_.isArray(opts.systemAssetDefinitions)) {
    opts.systemAssetDefinitions.forEach(function (sad) {
      self.assetManager.resolveAssetDefinition(sad) // will return Promise! need .done !
    })
  }
...
Wallet.prototype._addEventListeners = function () {
  ...
  self.blockchain.on('newBlock', function (hash, height) {
    self.emit('newBlock', hash, height)
  })

  // state
  // @todo: addTx, updateTx, revertTx, touchAddress, touchAsset
  ...
  _.each([self.blockchain], function (obj) {
  ...
  var notifyNeedSync = _.debounce(function () {
    if (!self.connector.isConnected() || syncQueue.length >= 2) {
      return
    }
  ...
  self.connector.on('connect', notifyNeedSync)
  self.blockchain.on('touchAddress', notifyNeedSync)
  // setInterval(notifyNeedSync, 5000)

  function subscribeAllAddressesAndSync () {
    self.isInitialized()
      .then(function () {
        return self.getAllAddresses()
      })
      .then(function (addresses) {
        return Promise.map(addresses, function (address) {
          return self.blockchain.subscribeAddress(address)
        })
      })
      .then(notifyNeedSync)
  ...
  self.on('initialize', subscribeAllAddressesAndSync)
  self.on('newAddress', subscribeAllAddressesAndSync)
...
Wallet.prototype.isInitializedCheck = function () {
  var self = this
  return self._withInitializeLock(function () {
    return self.configStorage.get('isInitialized')
  })
  .then(function (isInitialized) {
    if (!isInitialized) {
