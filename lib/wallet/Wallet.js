 ...
 * @param {string} [opts.network.name='ChromaInsight']
 * @param {Object} [opts.network.opts]
 * @param {boolean} [opts.network.autoConnect=true]
...
function Wallet(opts) {
  ...
  opts.networkName = opts.testnet ? 'testnet' : 'bitcoin'
  opts = _.merge({
    network: {
      name: 'ChromaInsight',
  ...
  var NetworkCls = blockchainjs.network[opts.network.name]
  self.network = new NetworkCls(opts.network.opts)
  if (opts.network.autoConnect) { self.network.connect() }
  ...
  self.blockchain = new BlockchainCls(self.network, blockchainOpts)
  ...
  self.walletEventNotifier.on('error', function (err) { self.emit('error', err) })
