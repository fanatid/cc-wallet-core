 ...
 * @return {Q.Promise}
 */
WalletStateManager.prototype.sync = function (addresses, again) {
  console.log('WSM.sync!')
  var self = this

  if (!again) this._syncEnter()

  return self.execute(function (walletState) {
    console.log('WSM.sync.execute')

    return walletState.getTxStateSet().autoSync(self._wallet.getBlockchain(), 
        addresses).then(function (newTxSS) {
        console.log('WSM.sync.newTxSSg')
